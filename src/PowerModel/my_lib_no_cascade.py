...
# Model data
def optimal_no_cascade_power(nodes, power, link1, link2, linkNum, Thr):
    flows = []
    cost = []
    Total_power = []
    ILP_gen_power = []
    ...
    flows, ILP_gen_power, ILP_load_power, cost, TotalPower = cascade_solution_best(nodes, power, link1, link2, linkNum, Thr)
    ...
    while (loop ==1):
        ...
        flows, ILP_gen_power, ILP_load_power, cost, TotalPower = cascade_solution_best(nodes, power, link1, link2, linkNum)
        if (loop ==2):
            loop =1
        else:
            loop = 0
...
def cascade_solution_best(nodes, power, link1, link2, linkNum, Thr):
    #Power_gen_t = []
    #Power_load_t = []
    my_flows = []
    ILP_gen = []
    ...
    w_l = 100
    w_f = 1
    Max_Thr = Thr#0.31#1.5
    Min_Thr = (-1)*Thr #-0.31#1.5
    P_g_max = 1
    P_g_min = -1
    P_l_max = -1
    P_l_min = 1
    ...
    if my_Model.status == GRB.status.OPTIMAL:
        ...
        print 'ILP_loaaaad Powwwwwwwwwwwwwwer:'
        print ILP_load_power
    #Find the flows:
        theEdges =[]
        """
        ...
        #for h in range(2, 3):
            ...
            theLoad = delta_load[h]*load_sol[h]
            #print theLoad
