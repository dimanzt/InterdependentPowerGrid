...
# Model data
def optimal_Power_Control(H_Working, H, nodes, power, link1, link2, reactances, linkNum, Thr):
    flows = []
    cost = []
    Total_power = []
    ILP_gen_power = []
    ...
    flows, ILP_gen_power, ILP_load_power, cost, TotalPower = Power_Control_solution(H_Working, H, nodes, power, link1, link2, reactances, linkNum, Thr)
...
def Power_Control_solution(H_Working, H, nodes, power, link1, link2, reactances, linkNum, Thr):
    #Power_gen_t = []
    #Power_load_t = []
    my_flows = []
    ILP_gen = []
    ...
    for h in range(0,NodeNum):#310):
        Teta[h] = my_Model.addVar(ub=5, lb=-5, vtype=GRB.CONTINUOUS, name='Theta%s'% (h))
    theEdges= []
    #for m in range(0, len(link1)):
    for e in H.edges():
        #i = link1[m]
    ...
    for k in range(NumGen, NumGen + NumPow): #(113, 210):
        my_Model.addConstr(P_l_t[k] == sum(Br[k-1, j-1]*Teta[j] for j in range(1, NodeNum)), 'LoadFlow_%s'% (k))
    my_Model.update()
    #for k in range(NumGen + NumPow, NodeNum):
    #    my_Model.addConstr(P_j_t[k] == ILP_jun[k- NumGen - NumPow], 'JunctionPow_%s'% (k))
    for k in range(NumGen + NumPow, NodeNum):
        my_Model.addConstr(P_j_t[k] <= ILP_jun[k- NumGen - NumPow]+ 0.1, 'JunctionPow_1_%s'% (k))
        my_Model.addConstr(P_j_t[k] >= ILP_jun[k- NumGen - NumPow]- 0.1, 'JunctionPow_2_%s'% (k))
    #for k in range(NumGen + NumPow, NodeNum): #(210, 310):
    #for x in H.nodes():
    #    if (x >1):
    #        if x in Junction:
    ...
    for k in range(NumGen + NumPow, NodeNum):
        my_Model.addConstr(P_j_t[k] == sum(Br[k-1, j-1]*Teta[j] for j in range(1,NodeNum)), 'JunctionFlow_%s'% (k))
        #my_Model.addConstr(ILP_jun[k- NumGen - NumPow] == sum(Br[k-1, j-1]*Teta[j] for j in range(1,NodeNum)), 'JunctionFlow_%s'% (k))
    my_Model.update()
    ...
    #my_Model.addConstr((quicksum(P_g_t[l] for l in range(0,113)) + quicksum(P_l_t[j] for j in range(113, 210))) <= 0.1, 'ConsumedGeneratedPower')
    my_Model.addConstr((quicksum(P_g_t[l] for l in range(0,NumGen)) + quicksum(P_l_t[j] for j in range(NumGen, NumGen + NumPow))) >= -0.1, 'ConsumedGeneratedPower')
    my_Model.addConstr((quicksum(P_g_t[l] for l in range(0,NumGen)) + quicksum(P_l_t[j] for j in range(NumGen, NumGen + NumPow))) <= 0.1, 'ConsumedGeneratedPower')
        ...
        if x in Load:
            ...
            if (len(neighbors) != 0):
                ...
                if (theEdges !=[]):
                    ...
                    #print Load
                    #print (len(Load))
                    my_Model.addConstr(sum(flows[l,f,g] for (l,f,g) in theEdges) == P_l_t[k+NumGen] , 'Load_FlowConservation_%s'% (k))
        ...
        if x in Junction:
            ...
            if (len(neighbors) != 0):
                ...
                if (theEdges !=[]):
                    ...
                    #print Load
                    #print (len(Load))
                    #my_Model.addConstr(sum(flows[l,f,g] for (l,f,g) in theEdges) == ILP_jun[k] , 'Junction_FlowConservation_%s'% (k))
                    my_Model.addConstr(sum(flows[l,f,g] for (l,f,g) in theEdges) == P_j_t[k+NumGen+NumPow] , 'Junction_FlowConservation_%s'% (k))
                    #my_Model.addConstrs((flows.sum(e,f,g)  == P_l_t[k] for (e,f,g) in theEdges for k in range(113,210)), "node")
                    my_Model.update()
                else: #elif(theEdges == []):
                    my_Model.addConstr( P_j_t[k] == 0 , 'Junction_FlowConservation_%s'% (k))
                    my_Model.update()
            else:
                my_Model.addConstr( P_j_t[k] == 0 , 'Junction_FlowConservation_%s'% (k))
    ...
    my_Model.setObjective(w_g*(sum(( ILP_gen[l] - P_g_t[l] ) for l in range(0,NumGen)))+w_l*(sum((P_l_t[l] - ILP_load[l- NumGen]) for l in range(NumGen,NumGen + NumPow))), GRB.MINIMIZE)
        ...
        for n in H_Working.nodes():
            if (len(H_Working.nodes()) > 10):
                if ( n <= 113):
                    if n in Generator:
                        k = Generator.index(n)
                        ILP_gen_sol.append(gen_sol[k])
                        ILP_gen_cont.append(gen_sol[k])
                    else:
                        ILP_gen_cont.append(power[n-1])
                        ILP_gen_sol.append(power[n-1])
        ...
        for n in H_Working.nodes():
            if (len(H_Working.nodes()) > 10):
                if (113 < n <= 210):
                    if n in Load:
                        k = Load.index(n)
                        ILP_load_sol.append(load_sol[k+NumGen])
                        ILP_load_cont.append(load_sol[k+NumGen])
                    else:
                        ILP_load_sol.append(power[n-1])
    ...
    print TotalPower
    return my_flows, ILP_gen_cont, ILP_load_cont, cost, TotalPower #ILP_gen_power, ILP_load_power, cost #ILP_flows#flows
