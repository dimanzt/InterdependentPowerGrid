...
for j in range(0, len(link1)):
    ...
    #for j in range(0, len(link1)):
        ...
        for i in range(0, len(link1)):
            ...
            #[flows, ILP_gen_power, ILP_load_power, cost, ThisTotalPower] = optimal_DC_Flow(nodes, power, link1, link2, reactances, linkNum, Thr)
            """
            #print 'MaxPower', MaxPower, 'i', i, 'j', j, 'r', r, 'removed_index', len(removed_index), 'length link1', len(link1), 'flows', len(flows)
            if (MaxPower >= abs(flows[i]) and (i not in removed_index)):
                Index = i
                MaxPower = abs(flows[i])
                print 'MaxPower', MaxPower, 'i', i, 'j', j, 'r', r, 'removed_index', len(removed_index), 'length link1', len(link1)
        ...
        if (len(removed_index) > 0):
            for t in removed_index:
                ...
                del linkNum[t]
                del reactances[t]
    [Newflows, ILP_gen_power, ILP_load_power, cost, ThisTotalPower] = optimal_DC_Flow(nodes, power, link1, link2,reactances, linkNum, Thr)
