    ...
    for x in range(0,1):
                    ...
                    for n in graph_des.nodes():
                        ...
                        if (len(hviet.nodes()) > 10):
                            if(i < 113) and (i not in Curr_Gray_Nodes):
                                ...
                                print 'I'
                                print i
                                print 'UUUUUUUUUUUUUUUUUUUUUUUUUUUUUILP_gen_power_no_mon'
                                print len(ILP_gen_power_no_mon)
                                power[i] = ILP_gen_power_no_mon[j]
                            if (113 <= i < 210) and (i not in Curr_Gray_Nodes):
                                power[i] = ILP_load_power_no_mon[j - len(ILP_gen_power_no_mon)]
                        else:
        ...
        while (loop ==1):
            ...
            while (f < len(flows)):# and (f < len(Original_flows)):# and (Dest_perc > 0.0):
                ...
                if (abs(flows[f]) >= Thr) and (abs(flows[f])> Max_Flow):# and (times < i):# and (i>20):# or (flows[f] != -0.31):
                    Max_Flow = abs(flows[f])
