        ...
        while (loop == 1):
            ...
            for m in removed_index:
                if m not in repairs:
                    ...
                    #[Newflows, ILP_gen_power, ILP_load_power, cost, TotalPower] = optimal_DC_Flow(nodes, power, New_link1, New_link2, New_reactances, linkNum, Thr)
                    if abs(Newflows[m])>= Max_flow  and (Resources[m] <= Stage_Resource):
                        Max_flow = TotalPower
                        Added_Index = m
            if Added_Index not in repairs:
                repairs.append(Added_Index)
                Old_Index = Added_Index
            if (Added_Index != []):
                Left_Resources = Stage_Resource - Resources[Added_Index]
            loop = 0
            for m in removed_index:
                if (Left_Resources >= Resources[m]) and (Added_Index != Old_Index):
                    loop = 1
