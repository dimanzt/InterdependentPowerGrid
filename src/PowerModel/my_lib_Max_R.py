        ...
        while(loop ==1):
            ...
            for m in removed_index:
                ...
                for c in repairs:
                    ...
                    del New_reactances[c]
                [Newflows, ILP_gen_power, ILP_load_power, cost, TotalPower] = optimal_DC_Flow(nodes, power, New_link1, New_link2, New_reactances, linkNum, Thr)
                print 'Left Resources:', Left_Resource, 'MinFlow', Min_flow, 'Added_index', Added_Index, 'Flowin Time', Flow_inTime, 'TotalPower:', TotalPower, 'Remove index', m, 'Size of Resources:', len(Resources), 'Resources[m]', Resources[m], 'Newflows', len(Newflows)
                if (((float(abs(TotalPower)- LastFlow)/float(Resources[m]))<= Min_flow) and (Resources[m] <= Left_Resource)):
                    Min_flow = (float(abs(TotalPower)- LastFlow)/float(Resources[m])) #TotalPower
                    Added_Index = m
                    print 'Min_Flow', Min_flow, 'Added_Index:', Added_Index
            repairs.append(Added_Index)
            Left_Resource = Left_Resource - Resources[Added_Index]
