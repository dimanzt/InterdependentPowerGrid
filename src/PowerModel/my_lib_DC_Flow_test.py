...
# Model data
def optimal_DC_Flow(nodes, power, link1, link2, reactances, linkNum):
    flows = []
    cost = []
    Total_power = []
    ILP_gen_power = []
    ...
    flows, ILP_gen_power, ILP_load_power, cost, TotalPower = DC_Flow_solution(nodes, power, link1, link2, reactances, linkNum)
...
def DC_Flow_solution(nodes, power, link1, link2, reactances, linkNum):
    #Power_gen_t = []
    #Power_load_t = []
    my_flows = []
    ILP_gen = []
    ...
    #for h in range(0,310):
        Teta[h] = my_Model.addVar(ub=1, lb=-1, vtype=GRB.CONTINUOUS, name='Theta%s'% (h))
    theEdges= []
    for m in range(0, len(link1)):
        i = link1[m]
        j = link2[m]
    ...
    my_Model.update()
    my_Model.addConstr(Teta[0] == 0, 'ReferenceVoltage')
    #for k in range(1,1):
    ##for k in range(1, 113):
    #k =1
    #my_Model.addConstr(P_g_t[k] == sum(Br[k-1, j-1]*Teta[j] for j in range(1, 310)), 'GenFlow_%s'% (k))
    #my_Model.addConstr(P_g_t[k-1] == sum(Br[k-1, j-1]*Teta[j] for j in range(1, 3)), 'GenFlow_%s'% (1))
    my_Model.update()
    for k in range(1, 3):
    #for k in range(113, 210):
        i#my_Model.addConstr(P_l_t[k] == sum(Br[k-1, j-1]*Teta[j] for j in range(1, 310)), 'LoadFlow_%s'% (k))
        my_Model.addConstr(P_l_t[k] == sum(Br[k-1, j-1]*Teta[j] for j in range(1, 3)), 'LoadFlow_%s'% (k))
    my_Model.update()
    #Diman commented
    ##for k in range(210, 310):
    ...
    ##my_Model.addConstr((quicksum(P_g_t[l] for l in range(0,113)) + quicksum(P_l_t[j] for j in range(113, 210))) <= 0.1, 'ConsumedGeneratedPower')
    my_Model.addConstr((quicksum(P_g_t[l] for l in range(0,1)) + quicksum(P_l_t[j] for j in range(1, 3))) >= -0.01, 'ConsumedGeneratedPower')
    my_Model.addConstr((quicksum(P_g_t[l] for l in range(0,1)) + quicksum(P_l_t[j] for j in range(1, 3))) <= 0.01, 'ConsumedGeneratedPower')
    ...
    my_Model.setObjective(w_g*(sum((P_g_t[l] - power[l] ) for l in range(0,1)))+w_l*(sum((P_l_t[l] - power[l]) for l in range(1,3))), GRB.MINIMIZE)
