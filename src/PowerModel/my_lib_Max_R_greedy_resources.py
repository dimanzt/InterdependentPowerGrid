...
def optimal_Max_R(nodes, power, link1, link2, reactances, linkNum, Resources, Stage_Resource, Thr, removed_edges, removed_power, removed_index, Original_link1, Original_link2, Original_reactances):
    ...
    [Flow_inTime, repairs, time] = Max_R_solution_backward(nodes, power, link1, link2, reactances, linkNum, Resources, Stage_Resource, Thr, removed_edges, removed_power, removed_index, Original_link1, Original_link2, Original_reactances, Added_Flows, NumRepairs)
...
def Max_R_solution_backward(nodes, power, link1, link2, reactances, linkNum, Resources, Stage_Resource, Thr, removed_edges, removed_power, removed_index, Original_link1, Original_link2, Original_reactances, Added_Flows, NumRepairs):
    repairs = []
    time = []
    ################################################################################################################
    New_link1 = []
    ...
    for i in range(0, N):
        ...
        while(loop ==1):
            ...
            for m in removed_index:
                ...
                [Newflows, ILP_gen_power, ILP_load_power, cost, TotalPower] = optimal_DC_Flow(nodes, power, New_link1, New_link2, New_reactances, linkNum, Thr)
                print 'Max Flow', Max_flow, 'Added Index', Added_Index, 'LeftResource', Left_Resource, 'FlowinTime', Flow_inTime, 'TotalPower:', TotalPower, 'Remove index', m, 'Size of Resources:', len(Resources), 'Resources[m]', Resources[m], 'Newflows', len(Newflows)
                if (((float(abs(TotalPower)- LastFlow)/float(Resources[m]))>= Max_flow) and (Resources[m] <= Left_Resource)):
                    Max_flow = (float(abs(TotalPower)- LastFlow)/float(Resources[m])) #TotalPower
                    Added_Index = m
                    print 'Max_flows', Max_flow,'Last Flow', LastFlow, 'Added_Index:', Added_Index, 'added index', Added_Index
            repairs.append(Added_Index)
            print 'Left_Resource', Left_Resource, 'Added_Index', Added_Index, 'Resources', Resources
            Left_Resource =int(Left_Resource) - int(Resources[Added_Index])
            #Flow_inTime.append(Max_flow)
            if Added_Index in removed_index:
                removed_index.remove(Added_Index)
            print 'The Added_Index:', Added_Index
            loop = 0
            for m in removed_index:
                if (Left_Resource >= Resources[m]):
                    loop = 1
        ...
        time.append(time_index)
        time_index = time_index + 1
        print 'time:', time_index, 'TotalPower', TotalPower
        Left_Resource = Left_Resource + Stage_Resource
        New_link1 = []
        New_link2 = []
        New_reactances = []
        New_link1 = deepcopy(Original_link1)
        ...
        New_reactances = deepcopy(Original_reactances)
        for c in removed_index:
            if c not in repairs: #c!=Added_Index:
                del New_link1[c]
                del New_link2[c]
                del New_reactances[c]
        #This_flow= []
        Newflows = []
        #[This_flow, ILP_gen_power, ILP_load_power, cost, TotalPower] = optimal_DC_Flow(nodes, power, New_link1, New_link2, New_reactances, linkNum, Thr)
        [Newflows, ILP_gen_power, ILP_load_power, cost, TotalPower] = optimal_DC_Flow(nodes, power, New_link1, New_link2, New_reactances, linkNum, Thr)
        Flow_inTime.append(TotalPower)
        LastFlow = abs(TotalPower)
    print 'Repairs:'
    print repairs
    print 'time:'
    print 'Flow during time:'
