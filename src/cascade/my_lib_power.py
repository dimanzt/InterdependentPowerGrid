...
def create_er_random():
    ...
    for net in Ghviet: #Ggarr:
        ...
        for node in Ggarr: #Ghviet:
            ...
            if mindist < dist:
                ...
                inter = node
        """
        inter = random.choice(Ggarr.nodes())
        #print 'Ggarr random choice:'
        #print inter
        if not g_inter.has_key(inter):
            #h_inter.setdefault(inter, [net])
            g_inter.setdefault(inter, [net])
    ...
    for net in Ggarr: #Ggarr:
        ...
        for node in Ghviet: #Ghviet:
            ...
            if mindist < dist:
                ...
                inter = node
        """
        inter = random.choice(Ghviet.nodes())
        #print 'Ghviet random choice:'
        #print inter
        if not h_inter.has_key(inter):
            #print 'Here, inter, net:'
            #print inter, net
    ...
    for i in Ggarr.nodes():
        ...
        if 'Latitude' not in Ggarr.node[i]:
            Ggarr.node[i]['Latitude']= gc_dict.get(i)[0]#random.randint(0,300)
    ################################
    nx.write_gml(Ghviet,"er1.gml")
    nx.write_gml(Ggarr,"er2.gml")
    f_hviet.close()
    f_hvietc.close()
    f_garr.close()
    f_garrc.close()
...
########################################################
###########################################################################
def create_ba_random():
    
    Ghviet = create_graph('ba', 1000, 10)
    Ggarr = create_graph('ba', 1000, 10)
    h_inter = {}
    g_inter = {}
    i = 0
    for net in Ghviet: #Ggarr:
...
#############################################################################################
def Destroy_Monitors_from_init(hviet, garr, h_inter, g_inter, init_removed_g1, init_removed_g2):
    #Outputs
    non_cont_h_nodes = []
    non_cont_h_edges = []
    des_garr_nodes = []
    ...
    Destroyed = 0
    #percentage = float(float(Destroyed) / float(len(garr.nodes())))
    #for destroyed_node in init_removed_g2:
    #while (percentage < Dest_perc):
    for destroy_node in init_removed_g2:
        #destroy_node = random.randint(0, len(garr.nodes())-1)
        if destroy_node not in des_garr_nodes:
            des_garr_nodes.append(destroy_node)
            Destroyed = Destroyed + 1
...
def get_graph_from_destroyed_graph(H, destroyed_edges):
    new_graph=nx.Graph(H)#MultiGraph(H)#Graph(H)
    edges_to_remove=[]
    nodes_to_remove=[]
    print 'Number of edges to be removed:'
    print len(destroyed_edges)
    print destroyed_edges
    for edge in new_graph.edges():
        source=edge[0]
        target=edge[1]
        #if ((source,target) in destroyed_edges):
        #    print 'this edge:'
        #    print (source, target)
        #weight =edge[2]
        if new_graph.has_edge(source,target):
            ...
            if arc in destroyed_edges or arc_reverse in destroyed_edges:
                ...
                print 'This Edge: HHHHHHHHHHHHHHHeeeeereeee'
                print arc
            """
            #for k in keydict:
    ...
    return new_graph, nodes_to_remove, edges_to_remove
###############################################################################
