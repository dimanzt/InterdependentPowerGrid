...
def cascading_failures_ping_pong(G1, G2, init_nodes,  h_inter, g_inter ):
    ...
    return removed_g1, des_g2_nodes
##########################################	
...
if __name__ == '__main__':
    ...
    for i in range(51):#51
        ...
        for j in range(it):
            ...
            ########################################
            """
            while (len(remove_nodes) <  min((num_nodes * (i*1.6) / N), num_nodes)):#num_nodes * i / N:
                r = random.randint(0, num_nodes)
                if r not in remove_nodes:
                    remove_nodes.add(r)
                    k += 1
                if r not in init_removed_g1:
                    init_removed_g1.append(r)
                    #k += 1
            ...
            Steps, removed_g1_edges, removed_g1_nodes, removed_g2, TotalPowerAllGraphs= Let_it_Cascade(hviet, garr, init_removed_g1_edges, init_removed_g1, init_removed_g2, h_inter, g_inter, i*1.6, N, j)
            """
            for n in removed_g1_nodes:
                if n not in init_removed_g1:
                    init_removed_g1.append(n)
            """
            num_removed_g1_edges = len(removed_g1_edges)
            ping_pong_r_g1, ping_pong_r_g2= cascading_failures_ping_pong(hviet, garr, init_removed_g1, h_inter, g_inter)
            loop =0
            """
            while (len(ping_pong_r_g1) != len(removed_g1_nodes) and (loop < 3)):
                loop =loop + 1 
            ...
            sum += len(ping_pong_r_g1)#len(init_removed_g1)#(removed_g1) #len(hviet)
            sum2 += len(ping_pong_r_g2)#len(init_removed_g2)#(removed_g2) #len(garr)
            ...
            #Diman
        g1_failed.append(len(init_removed_g1) / float(num_nodes))
        g2_failed.append(len(init_removed_g2) / float(num_nodes2))
        y.append(sum / float(num_nodes) / it)
        y2.append(sum2 / float(num_nodes2) / it)
        y3.append(sum_edges /float(num_edges) /it)
        y4.append((PowerDelivered) /it)
        x.append(1-i/50.0)
    print 'Printing x:'
    print x
